...
- LongStitch is enabled by default. It uses the curated HiFi reads produced by this workflow.
...
| Parameter | Description | Example |
| --- | --- | --- |
| `selected_assemblies`   | One or more assemblies to process after hifiasm. Allowed values: `{"primary", "hap1", "hap2"}`. Downstream outputs are written under assembly-specific subdirectories such as `results/fcs/assembly/<selected_assembly>/...` or `results/release/<genome_version>/nuclear/<selected_assembly>/...`. Release outputs are produced for every selected assembly. When Hi-C integration is enabled, these correspond to hifiasm `hic.p_ctg`, `hic.hap1.p_ctg`, and `hic.hap2.p_ctg` outputs, which are then scaffolded with YaHS and exported as Juicebox-ready contact-map files. After the configured scaffolding steps, each selected assembly is sorted by decreasing sequence length and renamed under `results/renamed/assembly/<selected_assembly>/...`; this renamed assembly is used by RepeatModeler, RepeatMasker, annotation, and release generation. Organelle release assets are staged separately under `results/release/<genome_version>/organelle/...` according to `oatk_organelle` and `organelle_annotation`. | `["primary", "hap1", "hap2"]` |
...
| `oatk_organelle`        | One or more organelles to assemble, specified as a YAML list: `["mitochondrion"]`, `["chloroplast"]`, or `["mitochondrion", "chloroplast"]`. Order does not matter; empty lists, duplicate organelles, strings, and aliases are rejected. Defaults to both organelles if omitted. | `["mitochondrion", "chloroplast"]` |
...
The workflow retains QC results for the pre-rename assembly stages. It also runs SeqKit statistics, sequence-length and GC-content plots, BUSCO, Merqury, a self-dotplot, TIDK, and a fresh HiFi read-depth analysis on the length-sorted assembly under `results/renamed/`. HiFi reads are mapped again so the depth table uses the renamed sequence IDs.
...
| `organelle_reference_cds_qc` | Optional reference CDS/protein QC for PMGA/PGA annotations. Set an organelle `reference_dir` to `null` to skip reference QC, reference product fill, and reference-inferred RNA-editing rescue for that organelle. When `reference_dir` is set, the pipeline emits pre/post RNA-editing QC tables, fills empty CDS `/product` qualifiers from high-confidence same-gene reference CDS/protein matches, and conservatively tries closely related reference CDS/protein evidence for residual invalid CDS after RNA-seq curation. Reference-inferred rescue is applied only when a C-to-U site directly rescues complete-CDS validation and the edited protein matches the best same-gene/product reference; it does not require HiFi/DNA support. `fix_hifi_frameshifts: true` is allowed only for chloroplast/PGA and may correct well-supported HiFi read-supported frameshift indels before rerunning PGA. `fix_feature_boundaries: true` is also chloroplast/PGA-only and may correct conservative reference-guided CDS/exon feature boundaries for strongly short PGA v2 CDS annotations without changing the chloroplast sequence; it defaults to `true` for chloroplast/PGA and candidate JSON is emitted even when automatic application is disabled. Chloroplast/PGA requires a reference directory because PGA v2 itself is reference-based. | `{chloroplast: {reference_dir: "plastid_reference", fix_hifi_frameshifts: false, fix_feature_boundaries: true}, mitochondrion: {reference_dir: null}}` |

For PMGA/mitochondrion, CDS `/translation` qualifiers are regenerated with the
pipeline's final translation table. If residual validation failures are limited
to an undetermined terminal start and/or stop, the curation step marks the CDS
location with `<` and/or `>`, refreshes the corresponding note, and emits
`/translation`; other residual validation failures remain untranslated for
manual review.
