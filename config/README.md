...
| `pga_v2_fix_chloroplast_sequence_frameshifts` | Optional: when `true`, PGA v2 CDS-length QC may edit the chloroplast FASTA sequence itself to correct only high-confidence HiFi read-supported frameshift indels, then rerun PGA v2. Keep `false` to report candidates without changing the sequence. | `false` |
