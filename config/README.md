...
| `selected_assemblies`   | One or more assemblies to process after hifiasm. Allowed values: `{"primary", "hap1", "hap2"}`. Downstream outputs are written under assembly-specific subdirectories such as `results/fcs/assembly/<selected_assembly>/...` or `results/submission/<selected_assembly>/...`. When Hi-C integration is enabled, these correspond to hifiasm `hic.p_ctg`, `hic.hap1.p_ctg`, and `hic.hap2.p_ctg` outputs, which are then scaffolded with YaHS and written under `results/yahs/assembly/<selected_assembly>/...` before RepeatMasker and later steps. | `["primary", "hap1", "hap2"]` |
...
| `hic_reads_r1`          | Optional: Hi-C read 1 input for hifiasm phasing and YaHS scaffolding. Set to `null` to disable Hi-C integration, or provide one or more FASTQ paths as a YAML list. Must be paired with `hic_reads_r2`. [See hifiasm docs](https://github.com/chhylp123/hifiasm?tab=readme-ov-file#hi-c-integration) | `["raw_data/hic_R1.fastq.gz"]` |
| `hic_reads_r2`          | Optional: Hi-C read 2 input for hifiasm phasing and YaHS scaffolding. Must match `hic_reads_r1` in length and order. | `["raw_data/hic_R2.fastq.gz"]` |
